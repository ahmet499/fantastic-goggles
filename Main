...
class Room:
    ...
    def create_dungeon(self, height, width, num_iterations):
        ...
        # Initialize random walls around the central area
        for y in range(1, height - 1):
            for x in range(1, width - 1):
                if x < central_start_x or x >= central_start_x + central_width or y < central_start_y or y >= central_start_y + central_height:
                    dungeon[y][x] = '#' if random.random() < 0.6 else ' '

        # Cellular automaton to create open areas and walls
    ...
    def place_doors(self):
        ...
        # Place doors one block inside the edges of the room
        doors.append((width // 2, 1))                        # Top edge
        doors.append((width - 2, height // 2))                # Right edge
        doors.append((width // 2, height - 2))                # Bottom edge
        doors.append((1, height // 2))                        # Left edge
...
def draw_dungeon(stdscr, dungeon, player_x, player_y, score, doors, current_room_coords, monsters):
    ...
    for y, row in enumerate(dungeon):
        for x, cell in enumerate(row):
            if (x, y) in doors:
                stdscr.addch(y, x, '+', curses.color_pair(4))  # Doors (color_pair(4))
            elif cell == '#':
                stdscr.addch(y, x, '#', curses.color_pair(1))  # Rocks (color_pair(1))
            elif cell == '$':
                stdscr.addch(y, x, '$', curses.color_pair(2))  # Gold (color_pair(2))
            else:
                stdscr.addch(y, x, ' ')

    for monster in monsters:
        stdscr.addch(monster.y, monster.x, 'M', curses.color_pair(6))  # Monster (color_pair(6))

    stdscr.addch(player_y, player_x, '@', curses.color_pair(3))  # Player (color_pair(3))
...
def draw_projectile(stdscr, projectile):
    stdscr.addch(projectile['y'], projectile['x'], '*', curses.color_pair(5))  # Projectile (color_pair(5))

class Monster:
    def __init__(self, dungeon):
        self.x, self.y = self.place_monster(dungeon)
        self.move_delay = 0.5  # Adjust this value to control the monster's speed
    ...
    def shoot_projectile(self, player_x, player_y):
        # For simplicity, the monster shoots towards the player on every move
        return {'x': player_x, 'y': player_y}

def main(stdscr):
    curses.curs_set(0)
    stdscr.timeout(100)  # Set a delay for getch() in milliseconds

    curses.init_pair(1, curses.COLOR_WHITE, curses.COLOR_BLACK)  # Rocks (white on black)
    curses.init_pair(2, curses.COLOR_YELLOW, curses.COLOR_BLACK)  # Gold (yellow on black)
    curses.init_pair(3, curses.COLOR_CYAN, curses.COLOR_BLACK)    # Player (cyan on black)
    curses.init_pair(4, curses.COLOR_MAGENTA, curses.COLOR_BLACK)  # Doors (magenta on black)
    curses.init_pair(5, curses.COLOR_RED, curses.COLOR_BLACK)      # Projectile (red on black)
    curses.init_pair(6, curses.COLOR_GREEN, curses.COLOR_BLACK)    # Monster (green on black)

    grid_size = 5  # Change this to set the grid size
    seed_offset = 1000  # Ensure seeds are distinct
    ...
    monsters = [Monster(current_room.dungeon) for _ in range(3)]  # Add 3 monsters
    ...
    while True:
        ...
        if (player_x, player_y) in current_room.doors:
            current_room_coords = (current_room_coords[0] + (player_x == 1) - (player_x == len(current_room.dungeon[0]) - 2),
                                   current_room_coords[1] + (player_y == 1) - (player_y == len(current_room.dungeon) - 2))
            if 0 <= current_room_coords[0] < grid_size and 0 <= current_room_coords[1] < grid_size:
                current_room = rooms[current_room_coords[0]][current_room_coords[1]]
                player_x, player_y = place_player(current_room.dungeon, current_room)
                score += 5
            else:
